...
class StaticExporter:
    ...
    def export(self):
        ...
        r1i = torch.randn(1, 16, 68, 120).to(self.device, self.precision)
        r2i = torch.randn(1, 20, 34, 60).to(self.device, self.precision)
        r3i = torch.randn(1, 40, 17, 30).to(self.device, self.precision)
        r4i = torch.randn(1, 64, 9, 15).to(self.device, self.precision)
        ...
        torch.onnx.export(
            ...
            input_names=['src', 'r1i', 'r2i', 'r3i', 'r4i'],  # 不需要导出downsample_ratio，直接使用默认值
